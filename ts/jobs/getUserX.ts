...
async function buildUserX(runner:EntityRunner, to:number, bus:string, face:string):Promise<UserX[]> {
	// runner 可以做本地数据库缓存，不一定每次都到中央服务器获取，减轻中央服务器压力
	let results = await centerApi.userxBusFace(to, bus, face);
	return results;
}

export async function getUserX(runner:EntityRunner, to:number, bus:string, face:string): Promise<number[]> {	
	// 如果发给指定用户
	...
	if (userXArr === undefined) {
		userXArr = await buildUserX(runner, to, bus, face);
